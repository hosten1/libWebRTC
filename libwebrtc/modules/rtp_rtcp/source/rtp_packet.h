...
//  absl::optional<typename Extension::value_type> GetExtension() const;
   typename Extension::value_type GetExtension() const;
...
//  return result;
//}
typename Extension::value_type RtpPacket::GetExtension() const {
   typename Extension::value_type result;
  auto raw = FindExtension(Extension::kId);
    if (raw.empty() || !Extension::Parse(raw, &result)){
