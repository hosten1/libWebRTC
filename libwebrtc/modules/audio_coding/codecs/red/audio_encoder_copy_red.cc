...
size_t GetMaxRedundancyFromFieldTrial() {
  constexpr char kRedTrialName[] = "WebRTC-Audio-Red-For-Opus";
    const bool use_red_optimization = webrtc::field_trial::IsEnabled(kRedTrialName);
    size_t redundancy = 0;
    if (use_red_optimization) {
        const std::string field_trial_string = webrtc::field_trial::FindFullName(kRedTrialName);
        if (sscanf(field_trial_string.c_str(), "Enabled-%zu", &redundancy) != 1 ||
            redundancy > 9) {
            return kRedNumberOfRedundantEncodings;
        }
    }else{
...
//  RTC_LOG(LS_WARNING)
//        << "lym AudioEncoderCopyRed::AudioEncoderCopyRed number_of_redundant_encodings ="
